...
func main() {
	...
	for i := 0; i < t; i++ {
		...
		decomposition := decomposeIntoRoundNumbers(n)

		_, _ = fmt.Fprintln(bufStdout, len(decomposition))
		for _, v := range decomposition {
...
func decomposeIntoRoundNumbers(n int) []int {
	...
	for n > 0 {
		digit := n % 10
		if digit != 0 {
			decomposition = append(decomposition, digit*(power/10))
		}
